...
//
//	LoadEnabled  →  Facts  →  one execution per row  →  merge  →  status
//
...
//
// Each row is executed exactly once over the complete snapshot. Collection
// facts perform any VDU or VNFC iteration in Go and return deterministic
// answers to GRL, so rules neither name scaling-created instances nor require
// the runner to re-evaluate a whole document for every entity.
//
